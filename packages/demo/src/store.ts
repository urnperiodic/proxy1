...
let DEFAULT_WISP_URL = import.meta.env.VITE_WISP_URL || "ws://localhost:3000/";
if (typeof window !== "undefined" && (DEFAULT_WISP_URL.includes("localhost") || DEFAULT_WISP_URL.includes("127.0.0.1"))) {
	if (window.location.hostname !== "localhost" && window.location.hostname !== "127.0.0.1") {
		const wsProto = window.location.protocol === "https:" ? "wss:" : "ws:";
		DEFAULT_WISP_URL = `${wsProto}//${window.location.host}/`;
	}
}
